//-----------------------------------------------------------
// Richard Ogin
// RPL compiler "global" definitions and the common classes
//    RPLEXCEPTION, LISTER, READER, CODE, and IDENTIFIERTABLE
//
...
public:
   //-----------------------------------------------------------
   RPLEXCEPTION(const char description[])
   //-----------------------------------------------------------
   {
...
//-----------------------------------------------------------
{
   ...
   strcpy(fullFileName,sourceFileName);
   strcat(fullFileName,".rpl");
   SOURCE.open(fullFileName,ios::in);
   if ( !SOURCE ) throw( RPLEXCEPTION("Unable to open source file") );
...
// index in [ 0,LOOKAHEAD ] where index = 0 means last GetNextCharacter() returned
   if ( (0 <= index) && (index <= LOOKAHEAD) )
      ...
      throw( RPLEXCEPTION("GetLookAheadCharacter() index out-of-range") );
}
...
//-----------------------------------------------------------
{
   if ( numberCallbacks <= CALLBACKSALLOWED )
      ...
      throw( RPLEXCEPTION("Too many callback functions") );
}
...
*/
   if ( identifiers > capacity )
      throw( RPLEXCEPTION("Identifier table capacity exceeded") );
   else
   {
      identifiers++;
...
{
/*
   RPL static data--global variable/constant definitions, string literals, and 
   ...
   RPL dynamic (frame-resident) data--subprogram module formal parameters and
...
//--------------------------------------------------
{
   ...
   if ( !STM ) throw( RPLEXCEPTION("Unable to open code file") );
}
...
//--------------------------------------------------
{
   ...
   EmitUnformattedLine("");
   EmitUnformattedLine(";------------------------------------------------------------");
   EmitUnformattedLine("; Heap space for dynamic memory allocation (to support future RPL syntax)");
   EmitUnformattedLine(";------------------------------------------------------------");
